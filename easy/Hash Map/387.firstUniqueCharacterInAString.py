# 題目：
    # 給妳一個字串，找出第一個只出現一次的字元，
    # 回傳它的索引位置（如果都沒有，回傳 -1）。

# 想法：
    # 感覺要兩個迴圈 第一個迴圈:先把s建立一個空的字典s_dict 然後進入迴圈for char in s:
...
# 第一種做法：字典存「次數與索引」+ 第二趟掃描「字典 s_dict」
class Solution:
    def firstUniqChar(self, s: str) -> int:
        s_dict = {}
        # 第一趟：統計每個字元出現幾次
        # enumerate()把字串（或 list），每個元素前面自動加上一個編號，從 0 開始。
...
# 第二種做法：字典只存「次數」+ 第二趟掃描「原字串 s」
class Solution:
    def firstUniqChar(self, s: str) -> int:
        s_dict = {}
        # 第一趟（統計）：只用字典記錄「每個字母出現的次數」，例如 'a': 2, 'b': 1
        for char in s:
            s_dict[char] = s_dict.get(char, 0) + 1
        # 第二趟（尋找）：重新從頭走訪一次原始字串 s，並拿當前字母去字典查次數
        # 因為題目要求的重點是「第一個（First）」出現的唯一字元！
        # 當我們按照 s 從頭到尾（從左到右）的順序檢查，
        # 就能確保最先被找到的那個，一定是第一個出現的。
