# 給你一個整數陣列 nums,請你判斷這個陣列裡面是否有任何數字出現超過一次。
# 如果有重複的數字,回傳 True;如果所有數字都是獨一無二的,回傳 False。

# 想法：假設nums = [1,2,3,1]。
# 我會使用set()，將nums的這個list丟到set()中確認，然後拿nums的list 跟set()比對長度len 如果一樣長 代表false 如果不一樣長 代表set()中有一樣的被刪去 回傳true
